...
export const metadata: Metadata = { title: "Overview – Driver Portal" };
...
export default function OverviewPage() {
  return (
    <div className="p-8 space-y-8">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Compliance Overview</h1>
      ...
      {nonCompliant.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-red-800 mb-3">Non-Compliant Drivers — Immediate Action Required</h2>
          <div className="space-y-2">
            {nonCompliant.map((d) => (
              <div key={d.driver} className="flex items-center justify-between bg-white border border-red-100 rounded-lg px-4 py-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Link href="/documents" className="text-xs font-medium text-red-700 underline underline-offset-2">
                    Resolve
                  </Link>
                </div>
              </div>
            ...
            {expiringDocs.map((item) => (
              <div key={item.driver + item.doc} className="flex items-start justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{item.doc}</p>
